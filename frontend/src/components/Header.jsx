      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 lg:h-18 items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            <Link to="/" className="flex items-center space-x-3 button-hover">
              <div className="h-10 w-10 lg:h-12 lg:w-12 rounded-xl bg-gradient-primary flex items-center justify-center animate-glow">
                <span className="text-primary-foreground font-bold text-sm lg:text-base">SS</span>
              </div>
              <div className="hidden sm:flex flex-col">
                <span className="font-bold text-lg lg:text-xl text-primary">Saral Seva</span>
                <span className="text-xs lg:text-sm text-muted-foreground -mt-1">Government Services</span>
              </div>
            </Link>
          </div>
          ...
          <nav className="hidden lg:flex items-center space-x-8 xl:space-x-10">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className="text-sm lg:text-base font-medium text-foreground hover:text-primary transition-all duration-200 relative group"
              >
                <span className="relative z-10">{item.name}</span>
                <div className="absolute inset-0 bg-primary/10 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 -m-2"></div>
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center space-x-2 lg:space-x-4">
            <Button 
              variant="ghost" 
              size="icon" 
              className="hidden md:inline-flex button-hover relative group"
            ...
            >
              ...
              {unreadNotifications > 0 && (
                <Badge 
                  className="absolute -top-1 -right-1 h-5 w-5 text-xs flex items-center justify-center bg-destructive text-destructive-foreground p-0 animate-pulse"
                >
                  {unreadNotifications}
                </Badge>
              )}
            ...
            <div className="relative hidden md:block language-dropdown">
              <Button 
                ...
                className="flex items-center gap-2 button-hover px-3 py-2"
              >
                <span className="text-lg lg:text-xl">{languageInfo.flag}</span>
                <span className="hidden lg:inline font-medium">{languageInfo.name}</span>
            ...
            {isAuthenticated ? (
              <div className="hidden md:flex items-center space-x-2 lg:space-x-3">
                <Button variant="ghost" size="icon" className="button-hover" asChild>
                  <Link to="/profile">
                    <User className="h-4 w-4 lg:h-5 lg:w-5" />
                  </Link>
                </Button>
                <Button variant="outline" size="sm" className="button-hover" onClick={handleLogout}>
                  <LogOut className="h-4 w-4 mr-2" />
                  <span className="hidden lg:inline">Logout</span>
                </Button>
              </div>
            ) : (
              <Button variant="government" size="sm" className="hidden md:inline-flex button-hover px-6 py-2" asChild>
                <Link to="/login">
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side="right" className="w-[320px] sm:w-[400px] custom-scrollbar">
